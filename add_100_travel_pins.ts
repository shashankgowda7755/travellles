...
import { Pool } from 'pg';
import { travelPins } from './shared/schema';
...
async function addTravelPins() {
  try {
    console.log('Adding 100 travel pins to database...');
    
    for (const pin of travelPinsData) {
      await db.insert(travelPins).values({
        name: pin.name,
    ...
    console.log('Successfully added 100 travel pins!');
  } catch (error) {
    console.error('Error adding travel pins:', error);
  } finally {
    await pool.end();
    process.exit(0);
  }
